...
var parseApplescript = require('parse-applescript');
...
async function runAppleScript(script) {
    ...
    return parseApplescript.parseAppleScript(stdout);
}

async function tellProcess(process, script) {
    const result = await runAppleScript(outdent.outdent `
...
async function toggleCheckbox(props) {
    let checkboxAction;
    switch (props.value) {
        case true:
    ...
    await tellProcess(props.element.applicationProcess, outdent.outdent `
...
async function clickElement(element) {
    await tellProcess(element.applicationProcess, outdent.outdent `
...
class ElementReference extends BaseElementReference {
...
function pathPartsToPathString(pathParts) {
    return pathParts.map((part) => part.fullName).join(' of ');
}
function pathStringToPathParts(pathString) {
...
async function getElements(processName, args = {}) {
    const elements = createElementReferences((await runAppleScript(outdent.outdent `
...
function createBaseElementReference(elementPathString) {
...
function createElementReferences(elementPathStrings) {
...
async function waitForElementExists({ elementReference, interval = 0.1, }) {
    await tellProcess(elementReference.applicationProcess, outdent.outdent `
			repeat until exists ${elementReference.pathString}
					delay ${interval}
			end repeat
		`);
}
async function waitForElementHidden({ elementReference, interval = 0.1, }) {
    await runAppleScript(outdent.outdent `
			tell application "System Events"
...
async function waitForElementMatch(windowName, elementMatcher, pWaitForOptions) {
    pWaitForOptions = {
        timeout: 5000,
        ...pWaitForOptions,
    };
...
async function getElementProperties(elementOrElements) {
    if (Array.isArray(elementOrElements)) {
        const elements = elementOrElements;
        if (elements.length === 0)
            return [];
        const properties = (await tellProcess(elements[0].applicationProcess, outdent.outdent `
				return {${elements
        ...
        const properties = (await tellProcess(element.applicationProcess, outdent.outdent `
...
async function inputKeystrokes(keystrokes) {
    await runAppleScript(`tell application "System Events" to keystroke ${JSON.stringify(keystrokes)}`);
}

async function waitForWindow({ windowName, processName, numAttempts = 30, interval = 0.1, }) {
    await runAppleScript(outdent.outdent `
			tell application "System Events"
				set i to 0
				repeat until exists window ${JSON.stringify(windowName)} of application process ${JSON.stringify(processName)}
					delay ${interval}
					set i to i + 1
...
async function reopenSystemPreferences() {
    await runAppleScript(outdent.outdent `
...
async function openSystemPreferencesPane({ paneId, anchor, windowName, }) {
    await runAppleScript(outdent.outdent `
...
async function giveAppPermissionAccess({ appName, permissionName, adminCredentials, }) {
...
*/
async function allowSystemSoftware() {
    await reopenSystemPreferences();
    await openSystemPreferencesPane({
        paneId: 'com.apple.preference.security',
        anchor: 'General',
...
async function getTableRows(tableElement) {
    const rowPathStrings = (await tellProcess(tableElement.applicationProcess, outdent.outdent `
			get rows of ${tableElement.pathString}
		`));
    return rowPathStrings.map((pathString) => createBaseElementReference(pathString));
}
async function selectTableRow(props) {
...
async function setTextFieldValue(element, text) {
    await tellProcess(element.applicationProcess, outdent.outdent `
			set value of ${element.pathString} to ${JSON.stringify(text)}
		`);
}
async function getTextFieldValue(element) {
    const textFieldValue = await tellProcess(element.applicationProcess, outdent.outdent `
			get value of ${element.pathString}
		`);
    return textFieldValue;
}

exports.BaseElementReference = BaseElementReference;
exports.ElementReference = ElementReference;
exports.allowSystemSoftware = allowSystemSoftware;
exports.clickElement = clickElement;
exports.createBaseElementReference = createBaseElementReference;
exports.createElementReferences = createElementReferences;
...
exports.giveAppPermissionAccess = giveAppPermissionAccess;
exports.inputKeystrokes = inputKeystrokes;
exports.openSystemPreferencesPane = openSystemPreferencesPane;
exports.reopenSystemPreferences = reopenSystemPreferences;
exports.runAppleScript = runAppleScript;
exports.selectTableRow = selectTableRow;
exports.setTextFieldValue = setTextFieldValue;
exports.tellProcess = tellProcess;
exports.toggleCheckbox = toggleCheckbox;
exports.waitForElementExists = waitForElementExists;
exports.waitForElementHidden = waitForElementHidden;
exports.waitForElementMatch = waitForElementMatch;
