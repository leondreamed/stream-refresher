...
var applescriptUtils = require('applescript-utils');
...
async function cliclickElement(element) {
    await applescriptUtils.runAppleScript(outdent.outdent `
		tell application "OBS" to activate
	`);
    const startStreamingButtonProperties = (await applescriptUtils.runAppleScript(outdent.outdent `
...
async function stopStreaming() {
    const stopStreamingButton = await applescriptUtils.waitForElementMatch('OBS', (element) => element.path.some((part) => part.name.includes('Stop Streaming')), { timeout: 30000 });
    await cliclickElement(stopStreamingButton);
    const youWillNotBeAbleToReconnectText = await applescriptUtils.waitForElementMatch('OBS', (element) => element.path.some((part) => part.name.includes('You will not be able to reconnect.')));
    const yesButton = youWillNotBeAbleToReconnectText
        .nextElement()
        .nextElement();
    await applescriptUtils.clickElement(yesButton);
}
